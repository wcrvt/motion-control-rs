...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let xcmd_st: [f64; manipulator::WORKSPACE_DIM] =
        [0.0, 1.0 / xcmd_freq / 2.0 * std::f64::consts::PI];
    ...
    for _ in 0..SLOOP_NUM {
        ...
        let xcmd: [f64; manipulator::WORKSPACE_DIM] = [
            if t > xcmd_st[0] {
                plant_init_x[0] + xcmd_amp[0] * 0.5 * (1.0 - (xcmd_freq * (t - xcmd_st[0])).cos())
            } else {
                plant_init_x[0]
            },
            if t > xcmd_st[1] {
                plant_init_x[1] + xcmd_amp[1] * 0.5 * (1.0 - (xcmd_freq * (t - xcmd_st[1])).cos())
            } else {
                plant_init_x[1]
            },
        ];
        let dx_cmd: [f64; manipulator::WORKSPACE_DIM] = [
            if t > xcmd_st[0] {
                xcmd_amp[0] * 0.5 * xcmd_freq.powi(1) * (xcmd_freq * (t - xcmd_st[0])).sin()
            } else {
                0.0
            },
            if t > xcmd_st[1] {
                xcmd_amp[0] * 0.5 * xcmd_freq.powi(1) * (xcmd_freq * (t - xcmd_st[1])).sin()
            } else {
                0.0
            },
        ];
        let ddx_cmd: [f64; manipulator::WORKSPACE_DIM] = [
            if t > xcmd_st[0] {
                xcmd_amp[0] * 0.5 * xcmd_freq.powi(2) * (xcmd_freq * (t - xcmd_st[0])).cos()
            } else {
                0.0
            },
            if t > xcmd_st[1] {
                xcmd_amp[1] * 0.5 * xcmd_freq.powi(2) * (xcmd_freq * (t - xcmd_st[1])).cos()
            } else {
                0.0
            },
        ];
        ...
        let ddtheta_ref: [f64; manipulator::JOINTSPACE_DIM] =
            plant.computed_torque_method(&ddx_ref, &ddx_null);
