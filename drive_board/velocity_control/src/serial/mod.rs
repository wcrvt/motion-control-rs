    ...
    pub fn new(p: &SerialPortInfo) -> Option<USBPort> {
        ...
        } = p
        {
            Some(USBPort {
                ...
                vid: upi.vid,
                pid: upi.pid,
                serial_number: match &upi.serial_number {
                    Some(i) => i.clone(),
                    None => String::from(""),
                },
                manufacturer: match &upi.manufacturer {
                    Some(i) => i.clone(),
                    None => String::from(""),
                },
                product: match &upi.product {
                    Some(i) => i.clone(),
                    None => String::from(""),
                },
            })
            ...
            let port_specified: Vec<SerialPortInfo> = i
                ...
                .filter(|i| {
                    ...
                    } = i
                    {
                        ...
                        if let Some(x) = dev.vid {
                            is_match &= upi.vid == x
                        };
                        if let Some(x) = dev.pid {
                            is_match &= upi.pid == x
                        };
                        if let Some(x) = dev.manufacturer {
                            if let Some(i) = &upi.manufacturer {
                                is_match &= i == x
                            } else {
                                is_match = false
                            }
                        };
                        if let Some(x) = dev.product {
                            if let Some(i) = &upi.product {
                                is_match &= i == x
                            } else {
                                is_match = false
                            }
