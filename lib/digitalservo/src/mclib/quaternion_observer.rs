...
impl <T: Float + Default + std::ops::AddAssign + std::ops::DivAssign> QuaternionObserver<T> {
  ...
  pub fn estimate(&mut self, omega_sense: &[T; 3], force: &[T; 3], acc_sense: &[T; 3], geomag_sense: &[T; 3]) -> ObserverOutput<T> {
    ...
    let dyn_acc_b_predict: Vector<T, 3> = Vector::from([force[0] / self.mb[0], force[1] / self.mb[1], force[2] / self.mb[2]]);

    let acc_sense: Vector<T, 3> = Vector::from(acc_sense);
    let geomag_sense: Vector<T, 3> = Vector::from(geomag_sense);
    let y_sense: Vector<T, 6> = Vector::from([acc_sense[0], acc_sense[1], acc_sense[2], geomag_sense[0], geomag_sense[1], geomag_sense[2]]);
    ...
    let y_est: Vector<T, 6> = Vector::from([acc_b_est[0], acc_b_est[1], acc_b_est[2], geomag_b_est[0], geomag_b_est[1], geomag_b_est[2]]);
    ...
    let q_hat_norm: T = (self.q_hat[0].powi(2) + self.q_hat[1].powi(2) + self.q_hat[2].powi(2) + self.q_hat[3].powi(2)).sqrt();
