...
//Use sequential data
pub struct DataBuffer <T, const P: usize, const Z: usize> where [(); P + Z + 1]:, [(); Z + 1]: {
  pub u: Vector<T, {Z + 1}>,
  pub x: Vector<T, P>,
...
impl <T: Float + Default, const P: usize, const Z: usize>DataBuffer<T, P, Z> where [(); P + Z + 1]:, [(); Z + 1]: {
  pub fn new() -> Self {
    Self {
      x: Vector::new(),
      u: Vector::new(),
  ...
  pub fn add(&mut self, u: T, x: T, y: T) {
    ...
    for i in 0..P { phi[i] = self.x[i] };
    for i in 0..(Z + 1) { phi[i + P] = self.u[i] };
