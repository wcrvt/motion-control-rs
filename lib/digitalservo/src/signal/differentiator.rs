...
pub struct Differentiator <T, const DIFF_ORDER: usize, const FILT_ORDER: usize> where [(); DIFF_ORDER + FILT_ORDER]: {
  ts: T,
...
impl<T: Float + Default, const DIFF_ORDER: usize, const FILT_ORDER: usize> Differentiator <T, DIFF_ORDER, FILT_ORDER>
  where [(); DIFF_ORDER + FILT_ORDER]:, [(); DIFF_ORDER + FILT_ORDER + 1]:
{

  pub fn new(ts: T, bandwidth: T) -> Self {

    /* gain vector which realizes multiple root */
    let pascal_coeff: [T; DIFF_ORDER + FILT_ORDER + 1] = pascal_triangle::<T, {DIFF_ORDER + FILT_ORDER + 1}>();
    ...
    //system matrix
    let a_11: Matrix<T, {DIFF_ORDER + FILT_ORDER}, {DIFF_ORDER + FILT_ORDER}> = jordan_block::<T, {DIFF_ORDER + FILT_ORDER}>(T::zero());
