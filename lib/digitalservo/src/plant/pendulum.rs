...
impl <T: Float + std::ops::AddAssign + std::fmt::Debug> Pendulum <T> {
  ...
  pub fn update(&mut self, iq_ref: T, tau_p: T) {
    ...
    self.d2xp[0] = self.d2xm - lp_h * (self.d2theta * self.d0theta.cos() - self.d1theta.powi(2) * self.d0theta.sin());
    self.d2xp[1] =  - lp_h * (self.d2theta * self.d0theta.sin() + self.d1theta.powi(2) * self.d0theta.cos());
    ...
    let f_resultant_m: T = self.mp * lp_h * (self.d2theta * self.d0theta.cos() - self.d1theta.powi(2) * self.d0theta.sin());
    let moment_p: T = self.mp * lp_h * (self.d2xm * self.d0theta.cos() + T::from(G).unwrap() * self.d0theta.sin());
