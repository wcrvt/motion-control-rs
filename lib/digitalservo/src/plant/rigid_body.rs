...
impl<T: Float + Default + std::ops::AddAssign + num_traits::Float + std::fmt::Debug> RigidBody<T> {
    ...
    pub fn update(&mut self, force: [T; JOINTSPACE_DIM], torque: [T; JOINTSPACE_DIM]) {
        ...
        // Prediction
        let dcm: DirectionCosineMatrix<T> =
            DirectionCosineMatrix::from(&self.quaternion.coordinate);
    ...
    pub fn computed_torque_method(&mut self, ddx_ref: &[T; QUATERNION_DIM]) -> [T; JOINTSPACE_DIM] {
        ...
        for index in indexes {
            let x: Matrix<T, 3, 3> = Matrix::from([
                self.jacobian[index[0]],
                self.jacobian[index[1]],
                self.jacobian[index[2]],
            ]);
        ...
        let ddx_ref: Vector<T, 3> =
            Vector::from([ddx_ref[index[0]], ddx_ref[index[1]], ddx_ref[index[2]]]);
        let jacobian: Matrix<T, 3, 3> = Matrix::from([
            self.jacobian[index[0]],
            self.jacobian[index[1]],
            self.jacobian[index[2]],
        ]);
        let djacobian: Matrix<T, 3, 3> = Matrix::from([
            self.djacobian[index[0]],
            self.djacobian[index[1]],
            self.djacobian[index[2]],
        ]);
        let dq: Vector<T, 3> = Vector::from([
            self.quaternion.velocity[index[0]],
            self.quaternion.velocity[index[1]],
            self.quaternion.velocity[index[2]],
        ]);
