...
impl<T: Float> ThreePhaseState<T> {
    ...
    pub fn transform_dq(&self, theta: T) -> DirectQuadrantState<T> {
        ...
        let phase_offset: T =
            T::from(2.0).unwrap() / T::from(3.0).unwrap() * T::from(std::f64::consts::PI).unwrap();
        DirectQuadrantState {
            d: c_1
                * ((theta).cos() * self.u
                    + (theta - phase_offset).cos() * self.v
                    + (theta + phase_offset).cos() * self.w),
            q: c_1
                * (-(theta).sin() * self.u
                    - (theta - phase_offset).sin() * self.v
                    - (theta + phase_offset).sin() * self.w),
...
impl<T: num_traits::Float> DirectQuadrantState<T> {
    ...
    pub fn transform_uvw(&self, theta: T) -> ThreePhaseState<T> {
        ...
        ThreePhaseState {
            ...
            v: c_1
                * ((theta - phase_offset).cos() * self.d - (theta - phase_offset).sin() * self.q
                    + c_2 * self.z),
            w: c_1
                * ((theta + phase_offset).cos() * self.d - (theta + phase_offset).sin() * self.q
                    + c_2 * self.z),
